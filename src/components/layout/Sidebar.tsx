  ...
  return (
    <aside className="w-60 shrink-0 bg-zinc-950 border-r border-white/5 flex flex-col h-screen sticky top-0">
      ...
      <nav className="px-2 mt-4 space-y-0.5 flex-1 overflow-y-auto">
        {navItems.map(({ icon: Icon, label, badge, active, path }) => (
          <Link
            to={path}
            key={label}
            className={`w-full flex items-center gap-3 px-3 py-2 rounded-md text-sm transition-colors ${active ? 'bg-white/5 text-white' : 'text-zinc-400 hover:text-zinc-200 hover:bg-white/[0.03]'
              }`}
          >
            <Icon className="w-[16px] h-[16px]" strokeWidth={1.75} />
            <span className="flex-1 text-left">{label}</span>
            {badge && (
              <span className="text-[10px] font-medium px-1.5 py-0.5 rounded bg-amber-500/15 text-amber-300 ring-1 ring-amber-500/20">
                {badge}
              </span>
            )}
          </Link>
